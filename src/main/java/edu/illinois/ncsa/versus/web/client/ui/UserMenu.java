...
public class UserMenu extends Composite {
    ...
    private void buildMenu(LoggedInEvent loggedInEvent) {
        ...
        options.insertItem(new MenuItem("Profile", new HistoryCommand("profile")), 0);
        options.insertSeparator(1);
        options.insertItem(new MenuItem("Logout", new HistoryCommand("logout")), 2);

        permissionUtil.doIfAllowed(Permission.VIEW_ADMIN_PAGES, new PermissionUtil.PermissionCallback() {

            @Override
            public void onAllowed() {
                // Insert admins options before logout
                options.insertItem(new MenuItem("Access control", new HistoryCommand("accessControl")), 2);
                options.insertItem(new MenuItem("Users management", new HistoryCommand("modifyPermissions")), 3);
                options.insertSeparator(4);
            }
        });
    ...
    private class HistoryCommand implements Command {
        ...
        public HistoryCommand(String link) {
            this.link = link;
        }
